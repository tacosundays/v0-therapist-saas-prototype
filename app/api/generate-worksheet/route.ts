...
const worksheetSchema = z.object({
  ...
  reflectionQuestions: z.array(z.string()).min(3).max(5).describe('Thought-provoking reflection questions'),
  exercises: z.array(z.object({
    title: z.string(),
    instructions: z.string(),
  })).min(2).max(4).describe('Practical exercises with clear instructions'),
  journalPrompts: z.array(z.string()).min(3).max(5).describe('Journal prompts for deeper self-exploration'),
})
  ...
  try {
    ...
    const { output } = await generateText({
      ...
      messages: [
        {
          role: 'system',
          content: `You are an expert mental health therapist creating evidence-based therapeutic worksheets. 
...
Format the educational content with proper paragraphs. Make exercises specific and achievable.`,
        ...
        {
          role: 'user',
          content: `Create a therapeutic worksheet with the following parameters:
...
1. An engaging title
2. Educational content explaining the topic and its relevance
...
5. 3-5 journal prompts for deeper exploration`,
    ...
    return Response.json({ worksheet: output })
  } catch (error) {
