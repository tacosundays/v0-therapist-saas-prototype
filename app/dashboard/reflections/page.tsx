...
export default function ReflectionsPage() {
  ...
  useEffect(() => {
    const loadReflections = async () => {
      ...
      try {
        ...
        const [clientsResult, reflectionsResult] = await Promise.all([
          supabase
            .from("clients")
  ...
  const filteredReflections = reflections.filter((reflection) => {
    ...
    const matchesClient = selectedClientId === "all" || reflection.client_id === selectedClientId
    const searchTarget = [
      reflection.title,
  ...
  return (
    <div className="space-y-8">
      ...
      {!isLoading && !error && filteredReflections.length === 0 && (
        <Card className="rounded-2xl">
          <CardContent className="py-12 text-center">
            <MessageSquare className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
            <p className="font-medium text-foreground">No reflections found</p>
            <p className="text-sm text-muted-foreground mt-1">Client journal entries will appear here after submission.</p>
          </CardContent>
        </Card>
      )}
      ...
      {!isLoading && !error && filteredReflections.length > 0 && (
        <div className="space-y-4">
          {filteredReflections.map((reflection) => {
            const client = clientsById.get(reflection.client_id)
            return (
              <Card key={reflection.id} className="rounded-2xl">
                <CardHeader>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <CardTitle className="text-base">{reflection.title || "Untitled reflection"}</CardTitle>
                      <p className="text-xs text-muted-foreground mt-1">
                        {client?.full_name || "Unknown client"} · {formatDate(reflection.created_at)}
                      </p>
                    </div>
